...
public class TypeUtils {
   ...
   */
  public static final WildcardType WILDCARD_ALL = wildcardType().withUpperBounds(Object.class)
      .build();
   ...
   * {@code Iterable<? extends Map<Integer, ? extends Collection<?>>>}, the parameter of
   * {@code TreeSet} is {@code ? extends Map<Integer, ? extends Collection<?>>}.
   * </p>
   *
  ...
  public static Map<TypeVariable<?>, Type> determineTypeArguments(final Class<?> cls,
    ...
    final Map<TypeVariable<?>, Type> typeVarAssigns = determineTypeArguments(midClass,
   ...
   */
  private static Type[] extractTypeArgumentsFrom(final Map<TypeVariable<?>, Type> mappings,
   ...
   * {@link WildcardType#getLowerBounds()} returns an empty array. Otherwise, it returns the result
   ...
   * {@link WildcardType#getUpperBounds()} returns an empty array. Otherwise, it returns the result
   * of {@link WildcardType#getUpperBounds()} passed into {@link #normalizeUpperBounds}.
   *
   * @param wildcardType the subject wildcard type, not {@code null}
  ...
  /**
   * Gets the raw type of a Java type, given its context. Primarily for use with
   * {@link TypeVariable}s and {@link GenericArrayType}s, or when you do not know the runtime type
   * of {@code type}: if you know you have a {@link Class} instance, it is already raw; if you know
   * you have a {@link ParameterizedType}, its raw type is only a method call away.
   *
   ...
   *
   * <p>
   * This method returns {@code null} if {@code type} is not assignable to {@code toClass}. It
   ...
   * {@link java.util.Hashtable Hashtable} are also both {@code Object}. In cases where the
   * interface specified by {@code toClass} is (indirectly) implemented more than once (e.g. where
  ...
  public static boolean isArrayType(final Type type) {
    return type instanceof GenericArrayType || type instanceof Class<?> && ((Class<?>) type)
        .isArray();
  }
   ...
   *
   * <p>
   * since {@code List} is a subinterface of {@code Collection}, this method will return the bounds
   * as if the declaration had been:
   * </p>
   *
   ...
   */
  private static Type[] unrollBounds(final Map<TypeVariable<?>, Type> typeArguments,
      final Type[] bounds) {
    Type[] result = bounds;
    int i = 0;
  ...
  private static String wildcardTypeToString(final WildcardType wildcardType) {
    ...
    } else if (upperBounds.length > 1 || upperBounds.length == 1 && !Object.class
        .equals(upperBounds[0])) {
