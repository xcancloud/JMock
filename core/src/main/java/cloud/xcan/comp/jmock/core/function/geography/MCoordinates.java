...
import static cloud.xcan.comp.jmock.api.i18n.JMockFuncDocMessage.DOC_MCOORDINATES_C1;
import static cloud.xcan.comp.jmock.api.i18n.JMockFuncDocMessage.DOC_MCOORDINATES_C2;
import static cloud.xcan.comp.jmock.api.i18n.JMockFuncDocMessage.DOC_MCOORDINATES_DESC;
...
public class MCoordinates extends AbstractMockFunction {
  ...
  public MCoordinates(Double minLng, Double maxLng, Double minLat, Double maxLat, Integer scale,
    ...
    if (ObjectUtils.isEmpty(maxLng) || maxLng > DEFAULT_MAX_LNG_VALUE) {
      ParamParseException.throw0(FPARAM_MAX_T, new Object[]{"maxLng", DEFAULT_MAX_LNG_VALUE});
    }
    ...
    if (ObjectUtils.isEmpty(maxLat) || maxLat > DEFAULT_MAX_LAT_VALUE) {
      ParamParseException.throw0(FPARAM_MAX_T, new Object[]{"maxLat", DEFAULT_MAX_LAT_VALUE});
    }
    if (ObjectUtils.isEmpty(scale) || scale < DEFAULT_SCALE_VALUE) {
      ParamParseException.throw0(FPARAM_MIN_T, new Object[]{"scale", DEFAULT_SCALE_VALUE});
    }
    if (scale > MAX_SCALE_VALUE) {
      ParamParseException.throw0(FPARAM_MAX_T, new Object[]{"scale", MAX_SCALE_VALUE});
    }
    if (minLng > maxLng) {
