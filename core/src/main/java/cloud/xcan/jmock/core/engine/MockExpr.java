...
public sealed interface MockExpr {
  ...
  /**
   * A function call expression, e.g., @Email() or @String(6).
   * Arguments can themselves be MockExpr (supporting nesting like @Repeat(@Email(), 3)).
   */
  ...
  /**
   * An array expression — wraps a function call to produce multiple values.
   * Generated when @Repeat(innerFunc, count) is parsed.
   */
   ...
   * Value can be a literal string or a nested MockExpr (for function-in-function).
   */
