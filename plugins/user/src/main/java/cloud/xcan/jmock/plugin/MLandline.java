...
import static cloud.xcan.jmock.api.support.utils.RandomStringUtils.randomNum;
import static cloud.xcan.jmock.plugin.UserDocMessage.DOC_CATEGORY_USER;
import static cloud.xcan.jmock.plugin.UserDocMessage.DOC_LANDINE_C1;
import static cloud.xcan.jmock.plugin.UserDocMessage.DOC_LANDINE_C2;
import static cloud.xcan.jmock.plugin.UserDocMessage.DOC_LANDINE_DESC;
...
public class MLandline extends AbstractMockFunction {
  ...
  public MLandline(Locale locale) {
    if (CHINA.equals(locale)) {
      ...
      String landLine = MessageResources.getString(UserDocMessage.DATA_LANDLINE, locale);
