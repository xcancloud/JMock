...
import static cloud.xcan.jmock.plugin.DateDocMessage.DOC_LOCALE_DATE_TIME_C2;
import static cloud.xcan.jmock.plugin.DateDocMessage.DOC_LOCALE_DATE_TIME_C3;
import static cloud.xcan.jmock.plugin.DateDocMessage.DOC_LOCALE_DATE_TIME_C4;
import static cloud.xcan.jmock.plugin.DateDocMessage.DOC_LOCALE_DATE_TIME_DESC;
import static cloud.xcan.jmock.plugin.DateDocMessage.DOC_LOCALE_DATE_TIME_PARAMETER_FORMAT;
import static cloud.xcan.jmock.plugin.DateDocMessage.DOC_LOCALE_DATE_TIME_PARAMETER_RANDOM;
...
public class MLocaleDateTime extends AbstractMockFunction {
  ...
  @JMockParameter(descI18nKey = DOC_LOCALE_DATE_TIME_PARAMETER_FORMAT, type = "String", defaultValue = "yyyy-MM-dd HH:mm:ss")
  private String format;

  @JMockParameter(descI18nKey = DOC_LOCALE_DATE_PARAMETER_ZONEID, type = "String", defaultValue = "Asia/Shanghai")
  private ZoneId zoneId;

  @JMockParameter(descI18nKey = DOC_LOCALE_DATE_TIME_PARAMETER_RANDOM, type = "Boolean", defaultValue = "false")
  ...
  public String mock() {
    if (!random) {
      ...
      return isFuture ? df.format(LocalDateTime.now(zoneId)
          .plusDays(RandomUtils.nextInt(0, 10 * 365)).plusSeconds(24 * 60 * 60))
          : df.format(LocalDateTime.now(zoneId)
              .minusDays(RandomUtils.nextInt(0, 10 * 365)).plusSeconds(24 * 60 * 60));
    }
  }
}
