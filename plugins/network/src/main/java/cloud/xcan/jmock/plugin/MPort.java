...
import static cloud.xcan.jmock.plugin.NetworkDocMessage.DOC_PORT_C1;
import static cloud.xcan.jmock.plugin.NetworkDocMessage.DOC_PORT_C2;
import static cloud.xcan.jmock.plugin.NetworkDocMessage.DOC_PORT_DESC;
import static cloud.xcan.jmock.plugin.NetworkDocMessage.DOC_PORT_PARAMETER_MAX;
import static cloud.xcan.jmock.plugin.NetworkDocMessage.DOC_PORT_PARAMETER_MIN;
...
public class MPort extends AbstractMockFunction {
  ...
  public MPort(Integer min, Integer max) {
    ...
    if (ObjectUtils.isEmpty(min)) {
      ParamParseException.throw0(PARAM_NOT_NULL_T, new Object[]{"min"});
    }
    if (min < DEFAULT_MIN_VALUE) {
      ParamParseException.throw0(PARAM_MIN_T, new Object[]{"min", DEFAULT_MIN_VALUE});
    }
    if (ObjectUtils.isEmpty(max)) {
      ParamParseException.throw0(PARAM_NOT_NULL_T, new Object[]{"max"});
    }
    if (max > DEFAULT_MAX_VALUE) {
      ParamParseException.throw0(PARAM_MAX_T, new Object[]{"max"});
    }
    if (min > max) {
